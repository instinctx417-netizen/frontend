...
export default function HRTicketsPage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user?.userType === 'hr' && !hasLoadedRef.current) {
  ...
  useEffect(() => {
    ...
    const handleTicketAssigned = (data: { ticket: any }) => {
      loadTickets(pagination?.page || 1);
      if (selectedTicket === data.ticket.id) {
        loadTicketDetails(data.ticket.id);
      }
    };
  ...
  const loadTickets = async (pageToLoad: number = 1) => {
    try {
      setLoading(true);
      setError('');
      const response = await hrApi.getAssignedTickets(statusFilter || undefined, pageToLoad, 10);
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      const response = await hrApi.updateTicketStatus(selectedTicket, newStatus);
      if (response.success) {
  ...
  const handleTicketClick = (ticketId: number) => {
    ...
    router.push(`/hr/dashboard/tickets?id=${ticketId}`);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-black">Assigned Tickets</h1>
        </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              <div className="p-4 border-b border-gray-200">
                <h2 className="text-lg font-bold text-black">My Assigned Tickets</h2>
              </div>
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <select
                          ...
                          className="px-2 py-1 text-xs font-medium rounded border border-gray-300 focus:outline-none focus:ring-2 focus:ring-black"
                        >
                          <option value="assigned">Assigned</option>
                          <option value="in_progress">In Progress</option>
                          <option value="resolved">Resolved</option>
                    ...
                    <button
                      onClick={() => {
                        ...
                        setMessages([]);
                        router.push('/hr/dashboard/tickets');
                      }}
