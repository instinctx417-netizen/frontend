...
import { adminApi, Interview, PaginationMeta, clientPortalApi } from '@/lib/clientPortalApi';
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button
            onClick={() => router.push('/admin/dashboard/interview-logs')}
                      ...
                      <td className="px-6 py-2 whitespace-nowrap">
                        <select
                          ...
                          className={`px-3 py-1 rounded-full text-xs font-medium border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-black ${
                            ...
                            (pendingStatusChanges[interview.id] || interview.status) === 'confirmed' ? 'bg-blue-100 text-blue-800' :
                            (pendingStatusChanges[interview.id] || interview.status) === 'cancelled' ? 'bg-red-100 text-red-800' :
                            (pendingStatusChanges[interview.id] || interview.status) === 'scheduled' ? 'bg-gray-100 text-gray-800' :
                      ...
                      <td className="px-6 py-2 whitespace-nowrap text-sm">
                        <div className="flex items-center gap-2 flex-wrap">
                          {interview.candidateUserId && (
                            <a
                              href={`/admin/dashboard/candidates/detail?id=${interview.candidateUserId}`}
                              target="_blank"
