...
export default function AdminStaffPage() {
  ...
  const loadStaff = async (pageToLoad: number = 1) => {
    try {
      setLoading(true);
      setError('');
      const response = await adminApi.getStaffMembers(pageToLoad, 10);
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {staff.map((member) => (
                    <tr key={member.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-600">{member.organizationName || 'N/A'}</td>
                      ...
                      <td className="px-6 py-2 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          ...
                          <a
                            href={`/admin/dashboard/candidates/detail?id=${member.id}`}
                            target="_blank"
                            rel="noopener noreferrer"
